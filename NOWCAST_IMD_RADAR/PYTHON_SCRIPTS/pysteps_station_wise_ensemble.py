...
# === CONFIGURATION ===
N_ENS_MEMBERS = 10
TEMPORAL_WINDOW_SIZE = 4  # Number of timesteps for interpolation window (40 minutes)
...
def collect_station_data(station, forecast_time, required_timesteps=6):
    """
...
def run_station_forecast(station, files, times, outdir, station_obs_dir):
    """Run multi-timestep ensemble forecast for a station"""
    logger.info(f"Starting multi-timestep ENSEMBLE forecast for station: {station}")
    logger.info(f"Input files: {len(files)}, Output directory: {outdir}, Ensemble members: {N_ENS_MEMBERS}")
    ...
    try:
        ...
        logger.info("Calculating optical flow for multi-timestep ensemble forecast")
        V = motion.get_method("LK")(R)
        logger.info(f"Optical flow calculated for {len(times)} timesteps")

        # Run ENSEMBLE extrapolation forecast
        logger.info(f"Running 6-timestep ensemble forecast with {N_ENS_MEMBERS} members")
        
        # Use STEPS ensemble nowcasting method
        steps = nowcasts.get_method("steps")
        
        # STEPS parameters for ensemble generation
        nowcast_kwargs = {
        ...
        ensemble_forecast = steps(R, V, 6, **nowcast_kwargs)
        logger.info(f"Ensemble forecast completed. Shape: {ensemble_forecast.shape}")
        logger.info(f"Expected shape: ({N_ENS_MEMBERS}, 6, {R.shape[1]}, {R.shape[2]})")

        # Save ensemble forecast timesteps for each member
        logger.info("Saving ensemble forecast timesteps")
        for member_idx in range(N_ENS_MEMBERS):
            member_dir = os.path.join(outdir, f"member_{member_idx:02d}")
            os.makedirs(member_dir, exist_ok=True)
            
            for timestep_idx in range(6):
                timestamp = times[-1] + timedelta(minutes=10 * (timestep_idx + 1))
                out_path = os.path.join(member_dir, f"forecast_{station}_member_{member_idx:02d}_{timestamp.strftime('%d%b%Y_%H%M')}_rf.tif")
                # Convert dBZ to rainfall rate
                out_arr = dbz_to_rf(ensemble_forecast[member_idx, timestep_idx].astype(np.float32))

                logger.debug(f"Saving member {member_idx:02d}, timestep {timestep_idx+1}/6 to: {os.path.basename(out_path)}")
                with rasterio.open(out_path, "w", **profile) as dst:
                    dst.write(out_arr, 1)
                
                logger.debug(f"Member {member_idx:02d}, timestep {timestep_idx+1} saved. Range: [{np.min(out_arr):.2f}, {np.max(out_arr):.2f}] dBZ")

        logger.info(f"✅ All ensemble forecasts saved successfully for {station}")
        # ====================
        # DETERMINISTIC EXTRAPOLATION FORECAST
        # ====================
        ...
        extrapolate = nowcasts.get_method("extrapolation")
        forecast = extrapolate(R[-1], V, 6)
        logger.info(f"6-timestep forecast completed. Shape: {forecast.shape}")

        logger.info("Saving individual deterministic forecast timesteps")
        for i in range(6):
            ...
            out_arr = dbz_to_rf(forecast[i].astype(np.float32))
            with rasterio.open(out_path, "w", **profile) as dst:
                dst.write(out_arr, 1)

        logger.info(f"✅ Deterministic forecast saved successfully for {station}")
...
if __name__ == "__main__":
 ...
 'jpr', 'srn', 'jot', 'mks', 'ldn', 'sur', 'bnh', 'jmu', 'kuf', 'mur', 'slp']
    ...
    for station_idx, station in enumerate(stations):
        ...
        files, times, success = collect_station_data(station, forecast_time, required_timesteps=6)
